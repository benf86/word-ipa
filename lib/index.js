...
app.get('/:word', function (req, res) {
    translate(sanitize(req.params.word)).then(
        function (out) {
            res.json(out);
        });
});
...
function persistToDictionary (db, wordIPAPair) {
    dictionary[wordIPAPair.word] = wordIPAPair.IPA;
    return new Promise(function (resolve, reject) {
...
/**
 * Get the IPA transcription of a word from the source list
    ...
    return new Promise(function (resolve, reject) {
        ...
        getIPAFromOD(word).then(
            function (transcription) {
                out.IPA = transcription;
                out.statusCode = 200;
                out.message = 'Success';
                out.source = 'Oxford Dictionaries';
                resolve(out);
                return;
            },
            function () {
                out.statusCode = 404;
                out.message = 'IPA transcription not available either locally or in OD';
                resolve(out);
                return;
            });
    })
    ...
    return new Promise(function (resolve, reject) {
        ...
        http.request({
            ...
            res.on('end', function () {
                ...
                out.word = word;
                out.IPA = result[1];
                persistToDictionary(mongoDatabase, out);
                resolve(result[1]);
            });
        }).end();
    });
 ...
 * @param  {str} str Client-supplied string to lookup
 * @return {str} String that consists only of a-zA-Z0-9 characters with
 * apostrophes and anything past stripped
 */
function sanitize (str) {
    return str.replace(/[^\w-'"]/g, '').split('\'')[0];
}